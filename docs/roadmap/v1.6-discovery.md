...
- **Evaluation**: The v1.5 data model already records the underlying inputs required for standard EVM calculations: Baseline Budget at Completion ($BAC$), Planned Baseline Cost ($PV$), Percent Complete for Earned Value ($EV$), and Actual Work Hours and Resource Billing Rates for Actual Cost ($AC$):
  - **Planned Value (PV)**: $\text{Budgeted Cost of Work Scheduled (BCWS)}$ derived from active baseline cost.
  - **Earned Value (EV)**: $\text{Budgeted Cost of Work Performed (BCWP)} = BAC \times (\% \text{ Complete} / 100)$.
  - **Actual Cost (AC)**: $\text{Actual Cost of Work Performed (ACWP)} = \sum (\text{Actual Work Hours} \times \text{Resource Rate})$. (Note: Actual Work is labor effort in hours; Actual Cost is financial expenditure in currency).
  - **Schedule Variance (SV)**: $EV - PV$.
  - **Cost Variance (CV)**: $EV - AC$.
  - **Schedule Performance Index (SPI)**: $EV / PV$.
  - **Cost Performance Index (CPI)**: $EV / AC$.
  - **Estimate at Completion (EAC)**: Primary forecasting methodology $BAC / CPI$ (typical performance), with fallback to $AC + (BAC - EV)$ (atypical variance).
