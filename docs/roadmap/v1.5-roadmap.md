...
1. **Model Decoupling**: Core engines (`SchedulingEngine`, `ResourceEngine`, `ProjectValidator`, `MarkdownAdapter`, `ProjectCommandManager`) must remain 100% pure TypeScript with zero Obsidian or DOM dependencies.
...
- **Evaluation**:
  - PERT diagrams are frequently requested by project management purists, but practical project managers spend 95% of their time in the Gantt and Task Sheet views.
...
- **Proprietary Sidecar Databases (SQLite/JSON blobs for task data)**: All project data must remain 100% human-readable Markdown and YAML frontmatter.
